...
It is non-stationary Markov process, with the property of returns being normally distributed, which results a log-normal distribution for the price series.  
...
It$\hat{\mathrm{o}}$ lemma allows us to write a formal solution to the stochastic differential equation above as
$$Y_t = Y_0 \mathrm{exp} \lbrace \left( \mu - \frac{\sigma^2}{2} \right)t + \sigma W_t  \rbrace.$$
The solution above is then a log-normally distributed random variable, with mean $\mathrm{E}[Y_t]=Y_0 e^{\mu t}$ and variance $\mathrm{Var}[Y_t]=Y_0^2 e^{2\mu t} \left( e^{\sigma^2 t} - 1 \right)$.

Here, we integrate the stochastic differential equation using the Euler-Maruyama method, which is described in more details in the vignette describing the Orstein-Uhlenbeck process. 

Let's see how to generate geometric Brownian motion path (or realizations):
...
n <- round(1/dt)
N <- length(Y1)
returns <- (Y1[(n+1):N]-Y1[1:(N-n)])/Y1[1:(N-n)]
mean(returns)
sd(returns)
```
